...
function certFrameSVG(w, h, color, accent){
  ...
  return `
    <svg class="doc-cert-frame-svg" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
      <path d="${outerPath} ${innerPath}" fill="${color}" fill-rule="evenodd"/>
      <path d="${accentTop}${accentRight} ${accentWave}${accentLeft}" fill="none" stroke="${accent}" stroke-width="0.9" opacity=".85"/>
      <circle cx="${t}" cy="${t}" r="2.6" fill="${accent}"/>
