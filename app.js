...
function openApprentiDetail(id){
  ...
  root.innerHTML = `
    ...
    <div class="dash-block">
      ...
      <button class="btn btn-outline btn-block" id="btn-print-attestation" style="margin-top:8px;">${a.motif==='stagiaire'?'Imprimer l\'attestation de stage':'Télécharger le diplôme (PDF paysage)'}</button>
    </div>
  ...
  document.getElementById('btn-print-attestation').addEventListener('click', async (e)=>{
    if(a.motif==='stagiaire'){ printAttestation(a); return; }
    const btn = e.currentTarget;
    const original = btn.textContent;
    btn.disabled = true; btn.textContent = 'Génération du PDF...';
    await diplomeTelechargerPDF(a);
...
/* Génère un vrai fichier PDF (A4 paysage, 297x210mm) du diplôme et déclenche
   son téléchargement directement, sans passer par la boîte de dialogue
   d'impression du navigateur. Nécessite jsPDF + html2canvas (chargés via CDN
   dans index.html, mis en cache par le service worker après la première
   visite en ligne, donc disponibles ensuite hors connexion). */
async function diplomeTelechargerPDF(a){
  ...
  holder.innerHTML = diplomeHtml(a);
  const target = holder.querySelector('.doc-page-diplome');
  ...
  try{
    ...
    console.error('Échec génération PDF diplôme', err);
